...
class DashboardScreen extends StatelessWidget {
        ...
        child: SafeArea(
          child: Consumer<CardProvider>(
            builder: (context, provider, child) {
              ...
              // ── Categorise cards ──────────────────────────────────────────
              final overdue =
                  provider.cards.where((c) {
                    if (c.isPaid || c.dueDate == null) {
                      return false;
                    }
              ...
              final upcoming =
                  provider.cards.where((c) {
                    if (c.isPaid || c.dueDate == null || c.spent <= 0) {
                      return false;
                    }
                    final due = DateTime.parse(c.dueDate!);
                    return due.isAfter(now) &&
                        due.isBefore(now.add(const Duration(days: 10)));
                  }).toList()..sort(
                    (a, b) => DateTime.parse(
                      a.dueDate!,
                    ).compareTo(DateTime.parse(b.dueDate!)),
              ...
              final thisMonthTxs = provider.transactions
                  ...
                  .toList();
              final totalTxThisMonth = thisMonthTxs.fold(
                0.0,
                (s, tx) => s + tx.amount,
              );
              ...
              for (var tx in provider.transactions) {
                spending[tx.category] =
                    (spending[tx.category] ?? 0) + tx.amount;
              }
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          if (paid.isNotEmpty) ...[
                            ...
                            ...paid.map(
                              (c) =>
                                  _buildBillItem(context, c, BillStatus.paid),
                            ),
                            const SizedBox(height: 24),
                          ],
  ...
  Widget _buildBillItem(
    ...
    DateTime? due = card.dueDate != null
        ? DateTime.tryParse(card.dueDate!)
        : null;
  ...
  Widget _buildEmptyBills(bool isDark) {
    return Padding(
      ...
      child: Center(
        child: Text(
          'No bills yet. Sync with Gmail to fetch them.',
          style: TextStyle(
